  ...
  try {
    return Response.json(
      await searchDuckDuckGo(
        q,
        'BRAVE_SEARCH_API_KEY is not configured; using the no-key DuckDuckGo Lite fallback.',
      ),
    );
  } catch (error) {
    ...
    return Response.json(
      {
        error:
          `${detail}. Add BRAVE_SEARCH_API_KEY to bolt.diy/.env.local for the primary search provider.`,
        results: [],
      },
      { status: 503 },
    );
